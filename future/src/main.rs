...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
    ...
    for _ in 0..n {
      ...
      let m = read.i64();
      let hour = ((s[0] as u8 - '0' as u8) * 10 + (s[1] as u8 - '0' as u8));
      let warimashi = if hour >= 22 {1.2} else {1.0};
      let mut sum = b;
      sum += std::cmp::max(0,(m - a + (c - 1))/c) * d;
     // println!("{}",sum);
      sum = (sum as f64 * warimashi).floor() as i64;
      ans += sum;
    }
    println!("{}",ans);
}
