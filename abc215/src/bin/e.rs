...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
    let n = read.usize();
    let mut xy:Vec<_> = (0..n).map(|_| (read.i64(),read.i64())).collect();
    xy.sort();
}
fn binary_search(xy:&Vec<(i64,i64)>) -> i64 {
    let (mut lb,mut ub) = (0,1e9 as i64 + 1);
    while ub - lb > 1 {
        let mid = (ub + lb) / 2;
        if judge(xy,mid) {
            lb = mid;
...
fn judge(xy:&Vec<(i64,i64)>,criteria:i64) -> bool {
    let mut flag = false;
    let mut n = xy.len();
    for i in 0..n {
        if xy[i].0 <= xy[n - 1].0 - criteria && (xy[i].1 <= xy[xy.len() - 1].1 - criteria || xy[i].1 >= xy[xy.len() - 1].1 + criteria) {
            flag = true;
        }else if xy[i].0 >= xy[n - 1].0 + 
    }
    flag
}
//use proconio::input;
