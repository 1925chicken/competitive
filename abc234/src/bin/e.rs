...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
    let t = read.i64() as i128;
    let mut ans = 1e20 as i128;
   	dfs(0,t,&mut ans);
    println!("{}", ans);
}
...
fn dfs(now: i128, t: i128, ans: &mut i128) {
    if now >= t {
        *ans = std::cmp::min(*ans, now);
        return;
    }
    if now >= 10 {
        let dig = now % 10;
        let digdig = now % 100 - dig;
        let diff = digdig - dig;
        if dig + diff < 10 && dig + diff >= 0 {
            dfs(now * 10 + dig + diff, t, ans);
        }
    } else {
        for i in 0..10 {
            dfs(now * 10 + i, t, ans);
