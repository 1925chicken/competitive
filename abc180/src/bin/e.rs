...
fn solve(read:&mut snio::Reader<std::io::StdinLock<'_>>) {
    let n = read.usize();
    let xyz:Vec<_> = (0..n).map(|_| (read.i64(),read.i64(),read.i64())).collect();
    let mut dp = vec![vec![1e9 as i64;n];1 << n];
    dp[0][0] = 0;
    for s in 1..(1 << n) {
        for v in 0..n {
            if s & (1 << v) != 0 {
                for j in 0..n {
                    let (x,y,z) = xyz[v];
                    let (xx,yy,zz) = xyz[j];
                    dp[s][v] = std::cmp::min(dp[s][v],dp[s - (1 << v)][j] + (x - xx).abs() + (y - yy).abs() + std::cmp::max(0,z - zz));
                }
            }
        }
    }
