...
fn solve(read:&mut snio::Reader<std::io::StdinLock<'_>>) {
    ...
    let mut comb = binomial_coefficient::BinomialCoefficient::new((n + m) as usize,1e9 as i64 + 7);
    let mut con = vec![mod_int::ModInt::new(0);(n + m) as usize];
    for i in 0..(n + m) as usize {
        let t = std::cmp::min((i as i64 + k as i64 + 1) / 2, n as i64);
        con[i] += comb.permutation(i as i64 + 1,t);
        if m as usize >= i + 1{
            con[i] += 1;
        }
    }
