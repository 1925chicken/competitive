...
fn solve(read:&mut snio::Reader<std::io::StdinLock<'_>>) {
    let n = read.usize();
    let k = read.usize();
    let c:Vec<_> = (0..n).map(|_| read.i64()).collect();
    let mut st = std::collections::HashMap::<i64,i64>::new();
    for i in 0..k {
        *st.entry(c[i]).or_insert(0) += 1;
    }
    let mut ans = st.len();
    for i in k..n {
        //println!("{:?}",st);
        if st.contains_key(&c[i - k]) {
        *st.entry(c[i - k]).or_insert(0) -= 1;
        }
        if st.contains_key(&c[i - k]) && *st.get(&c[i - k]).unwrap() == 0 {
            st.remove(&c[i - k]);
        }
        *st.entry(c[i]).or_insert(0) += 1;
        ans = std::cmp::max(ans,st.len());
    }
    println!("{}",ans);
}
fn main() {
