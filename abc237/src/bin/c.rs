...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
	...
	let mut cnt_from_front = 0;
	let mut t:Vec<_> = (0..s.len()).map(|i| s[s.len() - 1 - i]).collect();
	let mut cnt_from_back = 0;
	while cnt_from_front < s.len() && s[cnt_from_front] == 'a' {
		cnt_from_front += 1;
	}
	while cnt_from_back < s.len() && s[s.len() - 1 - cnt_from_back] == 'a' {
		cnt_from_back += 1;
	}
	if cnt_from_front > cnt_from_back {
		println!("No");
	}else if cnt_from_front == cnt_from_back {
		println!("{}", if s == t {"Yes"}else {"No"});
	}else {
		while cnt_from_front != cnt_from_back {
			t.push('a');
			cnt_from_front += 1;
		}
		let mut u:Vec<_> = (0..t.len()).map(|i| t[t.len() - 1 - i]).collect();
		println!("{}",if t == u {"Yes"}else {"No"});
	}
}
