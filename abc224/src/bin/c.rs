...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
	let n = read.usize();
	let xy:Vec<_> = (0..n).map(|_| (read.i64() as i128 ,read.i64() as i128)).collect();
	let mut cnt = 0;
	for i in 0..n {
		 for j in i + 1..n {
			 for k in j + 1..n {
				let mut tt = Vec::new();
				tt.push(calc(xy[i],xy[j]));
				tt.push(calc(xy[i],xy[k]));
				tt.push(calc(xy[j],xy[k]));
				tt.sort();
				if /*tt[2] - tt[1] - tt[0] > 0 && */ 4 * tt[1] * tt[0] > (tt[2] - tt[1] - tt[0]) * (tt[2] - tt[1] - tt[0]) {
					cnt += 1;
				}
						 }
		 }
...
fn calc (ab:(i128,i128),cd:(i128,i128)) -> i128 {
	 (ab.0 - cd.0) * (ab.0 - cd.0) + (ab.1 - cd.1) * (ab.1 - cd.1)
}
fn main() {
	let t = std::io::stdin();
