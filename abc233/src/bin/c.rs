...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
	let n = read.usize();
	let x = read.i64() as i128;
	let mut a = vec![Vec::new();n];
	for i in 0..n {
		let l = read.usize();
		for _ in 0..l {
			a[i].push(read.i64() as i128);
		}
	}
	let mut cnt = 0;
	for i in 0..a[0].len() {
		dfs(&a,&mut cnt,x,a[0][i],1);
	}
	println!("{}",cnt);
}
fn dfs(a:&Vec<Vec<i128>>,cnt:&mut i128,x:i128,now:i128,id:usize) {
	if id == a.len() || x < now {
	//	println!("{}",now);
		if x == now {
			*cnt += 1;
