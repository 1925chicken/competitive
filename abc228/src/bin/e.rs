...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
	set_mod_int(998244353 as i128);
	let n = read.i128();
	let _k = read.i128();
	let _m = read.i128();
	let mut k = mod_int::ModInt::new(_k);
	k = k.pow(n);
	let mut m = mod_int::ModInt::new(_m);
	m = m.pow(k);
	println!("{}",m.value());
}
...
pub mod binomial_coefficient {
	...
	impl BinomialCoefficient<ModInt> {
	   pub fn new(n:usize,_mod:i128) -> Self {
			set_mod_int(_mod);
			let a = ModInt::new(1i128);
			let mut _fact:Vec<ModInt> = vec![a;n + 1];
			let mut _inversion:Vec<ModInt> = vec![a;n + 1];
			for i in 1..=n{
				_fact[i] = _fact[i - 1] * i as i128;
			}
			_inversion[n] = _fact[n].internal_pow(_mod - 2);
			for i in (1..=n).rev(){
			   _inversion[i - 1] = _inversion[i] * i as i128;
			}
		BinomialCoefficient {
			fact:_fact,
		...
		pub fn combination(&mut self,n:i128,k:i128) -> ModInt{
			if k < 0 || k > n {
				return ModInt::new(0i128)    
			}
			self.fact[n as usize] * self.inversion[k as usize] * self.inversion[n as usize - k as usize]
 
		}
 
		pub fn permutation(&mut self,n:i128,k:i128) -> ModInt{
			if k < 0 || k > n{
				return ModInt::new(0i128)
			}
