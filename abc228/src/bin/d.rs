...
fn solve(read: &mut snio::Reader<std::io::StdinLock<'_>>) {
    let q = read.usize();
    let mut a = vec![-1; 2_usize.pow(20) + 1];
    a[0] = -2;
    ...
    for i in 0..q {
        ...
        let mut xi = read.i64();
        if t == 1 {
            let u = lower_bound(&a, -1);
           // println!("{}", u);
		   a[u] = xi;
        } else {
            println!("{}", a[(xi % n) as usize]);
        }
    }
}
use std::iter::Iterator;
#[allow(dead_code)]
fn upper_bound<T>(f: &Vec<T>, criteria: T) -> usize
//criteriaより大きい要素が最初にどこに現れるかを返す
where
    T: std::cmp::Ord,
{
    let las = f.len();
...
#[allow(dead_code)]
fn lower_bound<T>(f: &Vec<T>, criteria: T) -> usize
//criteria以上の要素が最初にどこに現れるかを返す
where
    T: std::cmp::Ord,
{
    let las = f.len();
